...
def verify_phone(phone: str) -> None:
    ...
    if db_helper.auth.get_verify_phone_record(phone):
        db_helper.auth.update_verify_phone_record(phone, pincode)
    else:
        db_helper.auth.create_verify_phone_record(phone, pincode)
...
def submit_pincode(phone: str, pincode: str) -> str:
    ...
    real_pincode = db_helper.auth.get_verify_phone_record(phone)
    if not real_pincode:
    ...
    if pincode != real_pincode:
        attempts = db_helper.auth.increment_attempts(phone)
        if attempts >= 3:
            db_helper.auth.delete_verify_phone_record(phone)
