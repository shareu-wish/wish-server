...
def make_deposit(user_id, station_id) -> bool:
    ...
    """
    payment_token = db_helper.payments.get_user_payment_token(user_id)
...
def _check_for_delays() -> None:
    ...
    """
    orders = db_helper.payments.get_orders_with_delays()
    for order in orders:
        res = write_off_for_delay(order['user_id'], order['id'])
        if res:
            db_helper.payments.set_order_delay_paid(order['id'])
