...
def _monitor_take_umbrella_timeout():
    ...
    timeouts = db_helper.station_controller.get_all_station_lock_timeouts()

    for timeout in timeouts:
        if timeout['type'] == 1:
            # Вернуть депозит пользователю
            try:
                payments.refund_deposit(db_helper.orders.get_order(timeout['order_id'])['user_id'])
                print("Отзываем депозит из-за таймаута, user_id:", db_helper.orders.get_order(timeout['order_id'])['user_id'])
            ...
            db_helper.orders.close_order(timeout['order_id'], state=2)
            db_helper.station_controller.delete_station_lock_timeout(timeout['id'])
        elif timeout['type'] == 2:
            mqttc.publish(f"wish/station{timeout['station_id']}/slot{timeout['slot']}/lock", "close", qos=1, retain=True)
            db_helper.station_controller.delete_station_lock_timeout(timeout['id'])
...
def on_message(client, userdata, msg):
    ...
    if topic_parts[1][:7] == "station" and topic_parts[2][:4] == "slot" and topic_parts[3]:
        ...
        if topic_parts[3] == "has_umbrella" and msg.payload.decode() == "n":
            timeout = db_helper.station_controller.get_station_lock_timeout_by_station_and_slot(station_id, slot_id)
            if timeout is not None:
                db_helper.stations.decrease_free_umbrellas_on_station(station_id)
                db_helper.station_controller.delete_station_lock_timeout(timeout['id'])
                sleep(2)
            ...
            timeout = db_helper.station_controller.get_station_lock_timeout_by_station_and_slot(station_id, slot_id)
            if timeout is not None:
                db_helper.stations.increase_free_umbrellas_on_station(station_id)
                db_helper.station_controller.delete_station_lock_timeout(timeout['id'])
                # sleep(0.5)
                mqttc.publish(f"wish/station{station_id}/slot{slot_id}/lock", "close", qos=1, retain=True)
                ...
                # Вернуть депозит
                payments.refund_deposit(db_helper.orders.get_order(timeout['order_id'])['user_id'])
                print("Возвращаем депозит, user_id:", db_helper.orders.get_order(timeout['order_id'])['user_id'])

                db_helper.orders.close_order(timeout['order_id'], station_id, slot_id)
...
def give_out_umbrella(order_id: int, station_id: int) -> int | None:
    ...
    db_helper.station_controller.set_station_take_umbrella_timeout(order_id, station_id, slot_with_umbrella)
...
def put_umbrella(order_id: int, station_id: int) -> int | None:
    ...
    db_helper.station_controller.set_station_put_umbrella_timeout(order_id, station_id, free_slot)
